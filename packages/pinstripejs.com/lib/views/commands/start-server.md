...
- **`--without-background-jobs`** (optional) - Skip starting the background job services for background job processing
...
2. **Background Job Processing** - Automatically starts the background job services for scheduled tasks
...
- **Automatic Bot Startup** - Starts the background job services for scheduled background jobs by default
...
- **`queue-background-job`** - Queue background jobs for processing
