...
Shared base framework for Pinstripe and Sartor. Provides runtime conventions — class composition, hooks, registries, service injection, file discovery, context propagation — extracted into a neutral package. Pinstripe and Sartor both depend on it; neither depends on the other.
...
`Context` is the root carrier passed through a request or command run. Children inherit from a root and propagate values down — used by Pinstripe for request-scoped state and by Sartor for command-scoped state.
