...
test('subclass can extend dslProps with custom DSL methods', async () => {
    const Greeter = Text.extend('Greeter').include({
        meta(){
            this.assignProps({ dslProps: [...this.dslProps, 'greet'] });
        },
        greet(){
            this.line('hello');
        }
    });
    assert.equal((await Greeter.render(({ greet }) => greet())).toString(), 'hello');
});
