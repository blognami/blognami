...
const CURRENCIES = (() => {
    ...
    for (const { isoCode, name } of currencies) {
        const symbol = Intl.NumberFormat('en', {currency: isoCode, style: 'currency'}).format(0).replace(/[\d\s.]/g, '');
        const minimumAmount = minimumAmounts[isoCode] || 1;
        out[isoCode] = { isoCode, name, symbol, minimumAmount };
    }
