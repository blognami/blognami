...
Sartor is an agent-orchestration framework: a CLI for launching agent sessions, composing markdown/JS commands, services, and playbooks. You're running as a Sartor agent — this playbook covers how to respond, author new entities, and drive the CLI from inside a session.
...
To drive the CLI from inside a session — listing commands, running them, or spawning sub-agents — read `using-sartor-cli.md`.
