...
Place the file under the project's `lib/commands/` directory (typically `.sartor/lib/commands/`). The filename must be snake_case with a `.js` extension:
...
- **`sandboxed`** (boolean, optional) — when `true`, the command runs inside a Sartor sandbox. Omit or set to `false` for unsandboxed commands.
...
- **`this.project`** — project instance (async getter). Provides `rootPath`, `sartorPath`, etc. Resolve with `const project = await this.project`.
...
```javascript
export { Command as default } from 'sartor';
```
