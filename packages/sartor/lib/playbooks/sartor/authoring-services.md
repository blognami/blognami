...
Place the file under the project's `lib/services/` directory (typically `.sartor/lib/services/`). The filename must be snake_case with a `.js` extension:
...
```javascript
export { ServiceFactory as default } from 'sartor';
```

Generator commands create this file with `skipIfExists: true` so it is written once and never overwritten. Reference shapes: `sartor/lib/service_factory.js` and `haberdash/lib/abstract_service_factory.js`.
...
- `sartor/lib/services/logger.js` — session logger with directory allocation.
- `sartor/lib/services/agent.js` — agent runner with provider dispatch.
- `sartor/lib/services/sandbox.js` — sandbox provider factory.
