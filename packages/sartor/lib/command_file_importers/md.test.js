...
import { Command, Playbook, importAll } from 'sartor';
import './md.js';
...
describe('Command FileImporter md', () => {
    ...
    describe('run', () => {
        ...
        const sessionDir = join(projRoot, '.sartor', 'logs', 'sessions', '0042');
        let savedMixins;
        ...
        it('emits a single tail-f log hint to stdout, nothing to the log', async () => {
            ...
            assert.match(stdout, /Log: tail -f .*\.sartor\/logs\/sessions\/0042\/index\.md/);
            assert.ok(!stdout.includes('Sartor run-banner session'));
            assert.ok(!logged.some(l => l.includes('Sartor run-banner session')));
        ...
        it('non-md session: no header block or sibling files', async () => {
            const nonMdSessionDir = join(projRoot, '.sartor', 'logs', 'sessions', '9999');
