...
export default {
    meta(){
        this.assignProps({
            description: 'Generates a new playbook directory in the project .sartor/lib/playbooks directory. If a playbook with this name already exists, its directory contents are copied locally so they can be customised, and hands off to an agent to write it.',
            sandboxed: true
        });
    ...
    async run(){
        ...
        const authoringDocAbsPath = join(dirname(Playbook.for('sartor').latestFilePath), 'authoring-playbooks.md');

        await generateDir(join(libRoot, 'playbooks'), async () => {
            await generateFile('_file_importer.js', { skipIfExists: true }, ({ line }) => {
                line("export { Playbook as default } from 'sartor';");
            });
