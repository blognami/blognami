...
export default {
    meta(){
        this.assignProps({
            description: 'Generates a new service file in the project .sartor/lib/services directory. If a service with this name already exists, its source is copied locally so it can be customised, and hands off to an agent to implement it.',
            sandboxed: true
        });
    ...
    async run(){
        ...
        const authoringDocAbsPath = join(dirname(Playbook.for('sartor').latestFilePath), 'authoring-services.md');

        await generateDir(join(libRoot, 'services'), async () => {
            await generateFile('_file_importer.js', { skipIfExists: true }, ({ line }) => {
                line("export { ServiceFactory as default } from 'sartor';");
            });
