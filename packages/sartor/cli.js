...
(async () => {
    ...
    if (project.exists && !project.insideSartor) {
        const sartorDir = project.sartorPath;
        let bin;
        let search = sartorDir;
        while (true) {
            const candidate = join(search, 'node_modules', '.bin', 'sartor');
        ...
        if (bin) {
            const result = spawnSync(bin, process.argv.slice(2), { stdio: 'inherit', cwd: sartorDir });
            process.exit(result.status ?? 1);
        } else {
            process.stderr.write(chalk.red('sartor binary not found in any node_modules/.bin/ above .sartor/ — run `npm install` in .sartor/ or in a workspace ancestor') + '\n');
            process.exit(1);
        }
    }
    ...
    if (project.insideSartor) {
        const entry = join(project.sartorPath, 'lib', 'index.js');
        if (existsSync(entry)) {
