...
Component.register('blognami-document', {
    meta(){
    ...
    get loadCacheNamespace(){
        return this.head.find('meta[name="blognami-load-cache-namespace"]')?.params.content ?? 'default';
    },
    ...
    get componentBase(){
        return this.head.find('meta[name="blognami-component-base"]')?.params.content;
    },
    ...
    async load(url = this.url.toString(), options = {}){
        ...
        if(isHashOnlyChange){
            ...
            await this.constructor.for('blognami-frame').prototype.load.call(this, url, options);
        }
