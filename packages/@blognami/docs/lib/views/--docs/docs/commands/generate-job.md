...
blognami generate-job cleanup_old_data

# Create an email notification job
blognami generate-job send_daily_reports

# Create a backup job
blognami generate-job database_backup
```

## Description

The `generate-job` command is a **code generation tool** that creates scheduled jobs for your Blognami application. This command:
