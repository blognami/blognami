...
```bash
blognami generate-service <name>
```
...
blognami generate-service user_auth

# Create an email notification service
blognami generate-service email_notification

# Create a payment processing service
blognami generate-service payment_processor
```

## Description

The `generate-service` command is a **service scaffolding tool** that creates business logic services in your Blognami application:
...
- Services are automatically compatible with Blognami's ServiceFactory
...
- **`generate-project`** - Create a new Blognami project with services directory
