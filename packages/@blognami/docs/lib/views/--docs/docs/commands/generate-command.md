---
menu:
    path: ["Commands", "generate-command"]
---
...
```bash
blognami generate-command <name>
```
...
blognami generate-command my-task

# Create a command with multiple words
blognami generate-command send-email

# Create a command with camelCase (will be converted to snake_case)
blognami generate-command sendNotification
```

## Description

The `generate-command` command is a **CLI scaffolding tool** that creates new command files in your Blognami application. It automatically:
...
4. **Access Blognami features** like database, services, and utilities
