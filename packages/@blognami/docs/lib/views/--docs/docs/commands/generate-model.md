...
```bash
blognami generate-model <name> [fields]
```
...
blognami generate-model user

# Create a model with fields
blognami generate-model user "name:string email:string age:integer"

# Create a blog post model
blognami generate-model post "title:string content:text published:boolean"
```
