...
```bash
blognami start-server [--host <host:port>] [--without-jobs]
```

### Parameters

- **`--host <host:port>`** (optional) - Host and port configuration (e.g. "127.0.0.1:3000"). Defaults to `BLOGNAMI_HOST` environment variable or "127.0.0.1:3000"
...
BLOGNAMI_HOST="127.0.0.1:4000" blognami start-server
```
...
blognami start-server --host "127.0.0.1:4000"
```
...
NODE_ENV=production blognami start-server
```
...
- **`generate-project`** - Create new Blognami projects that can be served
