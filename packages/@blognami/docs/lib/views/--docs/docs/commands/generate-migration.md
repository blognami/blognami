...
> **Note**: This command is implemented as part of the `@blognami/database` package and needs to be included in your project for it to be available.
...
```bash
blognami generate-migration [suffix] [--table <table_name>] [--fields <field_definitions>]
```
...
blognami generate-migration add_users_table

# Create a migration with table name inferred from suffix
blognami generate-migration add_columns_to_users

# Create a migration with explicit table and fields
blognami generate-migration add_user_fields --table users --fields "email:string created_at:datetime"

# Create a migration with multiple fields
blognami generate-migration setup_posts --table posts --fields "title:string content:text published:boolean"
```
