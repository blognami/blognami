...
```bash
npx blognami start-server              # With job worker (default)
npx blognami start-server --without-jobs  # Without job worker
npx blognami run-job --name cleanup  # Run manually
```
