...
```bash
npx blognami start-server                 # With job coordinator (default)
npx blognami start-server --without-jobs  # Without job processing
```

## Distributed Mode

For multi-server deployments, use the `@blognami/distributed-jobs` package.

### Installation

```bash
npm install @blognami/distributed-jobs
```
...
```javascript
// lib/index.js
import '@blognami/distributed-jobs';
```
