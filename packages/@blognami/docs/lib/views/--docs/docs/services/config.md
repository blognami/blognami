...
Access application configuration from `blognami.config.js`.
...
The `config` service loads and normalizes configuration from `blognami.config.js`. It provides defaults for database, mail, and server settings while supporting custom configuration properties.
