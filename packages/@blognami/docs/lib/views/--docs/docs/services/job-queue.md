...
When using `@blognami/distributed-jobs`, the leader node automatically moves jobs from the in-memory queue to the `distributedJobs` database table. This allows all worker nodes to claim and process jobs. See [jobCoordinator](/services/job-coordinator) for details.
