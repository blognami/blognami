            ...
            }),
            {
                ...
                async success(){
                    ...
                    if(!user) return that.renderHtml`
                        <span data-component="blognami-anchor" data-href="/sign_in/create_account?email=${encodeURIComponent(email)}&password=${encodeURIComponent(await that.database.site.generatePassword(email))}${optionalParams}"><script type="blognami">this.parent.trigger('click');</script></span>
                    `;
                        ...
                        ${() => {
                            if(redirectUrl){
                                return that.renderHtml`
                                    <span data-component="blognami-anchor" data-href="${redirectUrl}">
