...
describe('logger – session dir allocation', () => {
    ...
    beforeEach(() => {
        root = mkdtempSync(join(tmpdir(), 'cardoon-logger-'));
        savedEnv = process.env.CARDOON_PARENT_SESSION_DIR;
        delete process.env.CARDOON_PARENT_SESSION_DIR;
    });

    afterEach(() => {
        if (savedEnv === undefined) delete process.env.CARDOON_PARENT_SESSION_DIR;
        else process.env.CARDOON_PARENT_SESSION_DIR = savedEnv;
        rmSync(root, { recursive: true, force: true });
    });

    it('allocates under .cardoon/logs/sessions when env unset', async () => {
        const logger = await service.create.call(mockThis({ rootPath: root }));
        assert.strictEqual(logger.sessionDir, join(root, '.cardoon', 'logs', 'sessions', '0001'));
    });

    it('allocates under <parent>/sessions when CARDOON_PARENT_SESSION_DIR set', async () => {
        const parent = join(root, '.cardoon', 'logs', 'sessions', '0003');
        process.env.CARDOON_PARENT_SESSION_DIR = parent;
        const logger = await service.create.call(mockThis({ rootPath: root }));
    ...
    it('logger.log writes to index.md in the session dir', async () => {
        ...
        const indexPath = join(root, '.cardoon', 'logs', 'sessions', '0001', 'index.md');
