...
Place the playbook directory under the project's `lib/playbooks/` directory (typically `.cardoon/lib/playbooks/`). The directory name must be dashed-case:

```
lib/playbooks/my-workflow/index.md
```

The playbook name is the directory name. So `lib/playbooks/my-workflow/` registers as the `my-workflow` playbook. The canonical reference playbook is this very `cardoon` playbook — study its other siblings when in doubt.
...
The host `cardoon` playbook (the one this file belongs to) is the canonical multi-sibling example:

```
lib/playbooks/cardoon/
