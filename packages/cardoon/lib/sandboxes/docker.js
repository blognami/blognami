...
export default {
    ...
    async _resolveSettings(){
        ...
        const name = config.sandbox?.name ?? `cardoon-${projectSlug}-sandbox`;
        const tag = name;
    ...
    async start(){
        ...
        if (imageCheck.exitCode !== 0) {
            const build = await this.spinner.run(
                ...
                () => this._exec('docker', [
                    'build', '-f', join(projectRoot, '.cardoon', 'Dockerfile.sandbox'),
                    '-t', tag, join(projectRoot, '.cardoon')
                ]),
    ...
    async run(command, opts = {}){
        ...
        const env = { CARDOON_IN_SANDBOX: '1', ...(config.sandbox?.env ?? {}), ...opts.env };
        const args = ['exec'];
