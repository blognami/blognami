...
export default {
    ...
    async run(){
        ...
        const authoringDocAbsPath = join(dirname(Playbook.for('cardoon').latestFilePath), `authoring-${extension}-commands.md`);

        await generateDir(join(libRoot, 'commands'), async () => {
            await generateFile('_file_importer.js', { skipIfExists: true }, ({ line }) => {
                line("export { Command as default } from 'cardoon';");
            });
            ...
            const { exitCode } = await this.agent.run({
                ...
                systemPrompt({ playbooks, line }){
                    ...
                    if(useExistingFile){
                        ...
                        line();
                        line(`1. Refer to the cardoon playbook for the ${extension} command authoring contract.`);
