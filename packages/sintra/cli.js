...
(async () => {
    const { entryPath, localSintraPath, exists } = await Project.instance;
    const { argv, env, execPath } = process;
    const args = argv.slice(2);

    if (env.IS_LOCAL_SINTRA != 'true' && localSintraPath) {
        spawn(execPath, [localSintraPath, ...args], {
            env: { ...env, IS_LOCAL_SINTRA: 'true' },
            stdio: 'inherit'
        });
    } else {
        if(entryPath){
