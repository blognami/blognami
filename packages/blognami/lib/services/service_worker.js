...
// How long (ms) to trust a single /_blognami/_shell/version.json identity +
...
const MARKUP_NODE_TYPE = 'application/vnd.blognami.markup-node+json';
...
export default {
    ...
    async handleFetch(event){
        ...
        if(!this._appCheck || now - this._appCheck.at >= APP_CHECK_TTL){
            this._appCheck = {
                at: now,
                isApp: fetch('/_blognami/_shell/version.json', { cache: 'no-store' })
                    .then(async response => {
