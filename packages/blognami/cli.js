...
(async () => {
    const { entryPath, localBlognamiPath, exists } = await Project.instance;
    const { argv, env, execPath } = process;
    const args = argv.slice(2);

    if (env.IS_LOCAL_BLOGNAMI != 'true' && localBlognamiPath) {
        spawn(execPath, [localBlognamiPath, ...args], {
            env: { ...env, IS_LOCAL_BLOGNAMI: 'true' },
            stdio: 'inherit'
        });
    } else {
        if(entryPath){
